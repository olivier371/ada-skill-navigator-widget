...
An embeddable quiz widget for [Amsterdam Data Academy](https://www.amsterdamdataacademy.com) that guides visitors through 5 questions about their background and goals, then returns a personalised learning path recommendation.

This repo contains a working implementation that calls the Claude API live on every submission. However, the widget's output is fully predictable from a fixed input space — making a **non-AI alternative** both feasible and preferable for production. Both approaches are documented below.

---

## Table of contents

**Current implementation (live API)**
...
**Non-AI alternative (recommended)**

7. [Why the API is unnecessary](#why-the-api-is-unnecessary)
8. [What the lookup table looks like](#what-the-lookup-table-looks-like)
9. [Why not building blocks and string concatenation?](#why-not-building-blocks-and-string-concatenation)
10. [Roadmap to migrate](#roadmap-to-migrate)
...
1. **No rate limiting** — Anyone who can reach the server can spam `POST /api/analyze` indefinitely, burning through API credits. There is no authentication, no per-IP throttling, and no request cap.

2. **Open CORS policy** — `cors()` is applied with no configuration, meaning any origin can call `/api/analyze`. This is fine for a public widget but worth tightening if the endpoint ever does anything more sensitive.

3. **Loose input validation** — The `answers` object from the request body is mapped directly into the Claude prompt without sanitisation. Since it goes to Claude rather than a database or shell, the injection risk is low — but a caller could send unexpectedly large values or attempt prompt manipulation.

4. **Unguarded `JSON.parse`** — The Claude response is parsed with `JSON.parse` inside a `try/catch` at the outer level, but the `.replace()` stripping of markdown fences (`` ```json `` etc.) runs before the parse without its own guard. If Claude returns something unexpected, this will throw and the outer catch will return a 500. Not a security issue, but a stability one.

**Overall:** safe to run locally with no changes. Before putting real traffic through it, rate limiting (risk #1) is the most important thing to address.

---

## Non-AI alternative (recommended)

### Why the API is unnecessary
...
**6 × 4 × 4 × 5 × 4 = 1,920 possible combinations.** That's a small, fully enumerable set. The Claude prompt asks for a fixed JSON structure — it isn't having a conversation or doing anything that requires real-time inference. This is a decision tree wearing an AI costume.

Replacing the live API call with a pre-generated lookup table eliminates every meaningful downside of the current setup:

| | Live API (current) | Pre-generated lookup table |
|---|---|---|
| **Response time** | ~1–3s (network + inference) | Instant |
| **Cost** | Per-request API charges | None at runtime |
...
```json
{
  "analyst|none|none|job|low": {
    "headline": "Start with the fundamentals of data",
    "summary": "You need a solid foundation before applying for data roles. With 1-3 hours per week you'll build the right skills step by step.",
    "level": "Starter",
    "recommended_track": "Python for Data Analysis",
    "track_description": "A practical 6-week course where you'll learn to work with real datasets.",
    "learning_path": [
      { "step": 1, "title": "Python basics", "duration": "2 weeks", "description": "Variables, loops and functions in a data context." },
      { "step": 2, "title": "Exploring data with pandas", "duration": "2 weeks", "description": "Loading, filtering and summarising datasets." },
      { "step": 3, "title": "Visualisation and communication", "duration": "2 weeks", "description": "Creating charts and presenting your findings clearly." }
    ],
    "skills_to_gain": ["Python", "pandas", "data visualisation", "analytical thinking"],
    "cta_text": "View Python for Data Analysis"
  },
...
### Why not building blocks and string concatenation?
...
- **Copy doesn't compose cleanly.** Sentence flow and natural phrasing mean that fragments that read fine in isolation produce awkward copy in some combinations. You won't catch all of them until a real person reads the output.
...
Regardless of which approach you run with, these improvements apply:

### Stability
- Wrap the `JSON.parse` call in its own try/catch with a meaningful fallback *(current implementation only)*
- Add a timeout to the Claude API fetch so a slow response doesn't hang the server *(current implementation only)*

### Security
- Add per-IP rate limiting (e.g. `express-rate-limit`) on `/api/analyze` *(current implementation only)*
- Restrict CORS to known ADA domains once the embed URL is fixed
- Validate and truncate incoming `answers` values *(current implementation only)*

### Product
- Track quiz completions and recommended tracks (anonymised) for ADA to understand visitor intent
- Add Dutch localisation alongside English
...
- Add a `.env.example` file so setup is self-documenting *(current implementation only)*
